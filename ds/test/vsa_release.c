...
{	
	...
	first_aloc = VsaAlloc(vsa1, 40);
	
	printTest("Is LargestChunk right while linear allocing... after alloc1 ", !(VsaLargestChunk(vsa1) == 80));
	
	second_aloc = VsaAlloc(vsa1, 24);
	
	printTest("Is LargestChunk right while linear allocing... after alloc2 ", !(VsaLargestChunk(vsa1) == 40));
	...
	printTest("Is LargestChunk right after freeing middle... free alloc2 ", !(VsaLargestChunk(vsa1) == 24));
	...
	strcpy(second_aloc, "abcdefghijklmno");
	
	printTest("Did usage of second aloc go as expected?", strcmp(second_aloc, "abcdefghijklmno"));
	
	VsaFree(third_aloc);
	
	printTest("Is LargestChunk right after defraging fragmantation... free alloc3 ", !(VsaLargestChunk(vsa1) == 48));
	
	third_aloc = VsaAlloc(vsa1,48);
	
	printTest("Check we can use defraged memory", !(VsaLargestChunk(vsa1) == 0));
	
	strcpy(third_aloc, "abcdefghijklmnopqrstuvwxyz0123456789abcdefghijk");
	
	printTest("Did usage of third aloc go as expected?", strcmp(third_aloc, "abcdefghijklmnopqrstuvwxyz0123456789abcdefghijk"));
	
	printTest("Did second aloc change becuase of bad treatment?", strcmp(second_aloc, "abcdefghijklmno"));
	...
	printTest("Check if we defrag right next to end", !(VsaLargestChunk(vsa1) == 40));
	...
	printTest("Does LargestChunk compare right between free blocks", !(VsaLargestChunk(vsa1) == 40));
	
	first_aloc = VsaAlloc(vsa1, 40);
		
	printTest("Does LargestChunk stay right after smaller free alloced", !(VsaLargestChunk(vsa1) == 40));
	
	VsaFree(second_aloc);
	
	printTest("LargestChunk after defrag alloc2 and alloc3 space", !(VsaLargestChunk(vsa1) == 80));
	...
	printTest("LargestChunk after defrag alloc1 and alloc2 space", !(VsaLargestChunk(vsa1) == 80));
	...
	first_aloc = VsaAlloc(vsa2, 38);
	second_aloc = VsaAlloc(vsa2, 22);
	...
	printTest("Check if we defrag right next to end", !(VsaLargestChunk(vsa2) == 40));
	...
	printTest("Does LargestChunk compare right between free blocks", !(VsaLargestChunk(vsa2) == 40));
	
	first_aloc = VsaAlloc(vsa2, 33);
		
	printTest("Does LargestChunk stay right after smaller free alloced", !(VsaLargestChunk(vsa2) == 40));
	
	VsaFree(second_aloc);
	
	printTest("LargestChunk after defrag alloc2 and alloc3 space", !(VsaLargestChunk(vsa2) == 80));
	
	second_aloc = VsaAlloc(vsa2, 20);
	
	third_aloc = VsaAlloc(vsa2, 36);
	
	VsaFree(first_aloc); 
	
	VsaFree(second_aloc);
	
	printTest("LargestChunk after defrag alloc1 and alloc2 space", !(VsaLargestChunk(vsa2) == 80));
